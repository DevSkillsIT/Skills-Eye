...
### Fases de Execução (Conforme Solicitado)
- **Fase 1 + 2** (SIMULTÂNEO): Integrar engine + Remover form_schema
- **Fase 3**: Adicionar rotas e menu
- **Fase 4**: Atualizar subtítulos das páginas
...
### ⚠️ DETALHES TÉCNICOS ESPECÍFICOS (Análise do Código Atual)

#### Backend - monitoring_types_dynamic.py (990 linhas)
**Linhas a REMOVER:**
- Linhas 193-348: Função `_infer_category_and_type()` (155 linhas hardcoded)
- Linhas 374-391: Função `_format_display_name()`
- Linhas 394-406: Função `_format_category_display_name()`

**Linhas a MODIFICAR:**
- Linha 169: `category, type_info = _infer_category_and_type(job_name, job)` → Usar engine
- Linha 351-371: `_extract_blackbox_module()` → MANTER (ainda necessária)
...
#### Backend - categorization_rules.py (455 linhas)
**Classes a REMOVER:**
- Linhas 64-76: Classe `FormSchemaField`
- Linhas 79-84: Classe `FormSchema`

**Campos a REMOVER dos Models:**
- Linha 94: `form_schema: Optional[FormSchema] = Field(None, ...)` em `CategorizationRuleModel`
- Linha 106: `form_schema: Optional[FormSchema] = None` em `RuleCreateRequest`
- Linha 117: `form_schema: Optional[FormSchema] = None` em `RuleUpdateRequest`

**Endpoints a MODIFICAR:**
- Linhas 167-248: POST - Remover form_schema do new_rule (linha 221)
- Linhas 256-349: PUT - Remover form_schema da atualização (linhas 317-318)

#### Backend - categorization_rule_engine.py (444 linhas)
**Linha a REMOVER:**
- Linha 61: `self.form_schema = rule_data.get('form_schema', None)` na classe `CategorizationRule`

**Método `categorize()` (linhas 262-340):**
- Já retorna `(category, type_info)` corretamente
- NÃO retorna form_schema (correto!)
- Usa método `_format_display_name()` interno (linhas 365-396) - MANTER

#### Frontend - MonitoringRules.tsx (720 linhas)
**Interfaces a REMOVER:**
- Linhas 57-75: Interfaces `FormSchemaField` e `FormSchema`

**Campos a REMOVER:**
- Linha 88: `form_schema?: FormSchema;` em `CategorizationRule`

**O modal de edição (linhas 548-677) NÃO TEM campos de form_schema** - verificado que já está correto!

#### Frontend - App.tsx (281 linhas)
**Rotas EXISTENTES (linhas 239-243):**
...
**Rotas a ADICIONAR após linha 243:**
```tsx
<Route path="/monitoring/infrastructure-exporters" element={<DynamicMonitoringPage category="infrastructure-exporters" />} />
<Route path="/monitoring/hardware-exporters" element={<DynamicMonitoringPage category="hardware-exporters" />} />
<Route path="/monitoring/network-devices" element={<DynamicMonitoringPage category="network-devices" />} />
...
**Menu Items a MODIFICAR (linhas 52-185):**
- Remover `/monitoring/rules` do menu Monitoramento (linhas 119-123)
- Adicionar ao menu Configurações (após linha 155)

#### Frontend - DynamicMonitoringPage.tsx (1402 linhas)
**Constante a ATUALIZAR (linhas 97-104):**
```typescript
const CATEGORY_DISPLAY_NAMES: Record<string, string> = {
  ...
  'network-devices': 'Dispositivos de Rede',       // ← ADICIONAR
  'custom-exporters': 'Exporters: Customizados',   // ← ADICIONAR
};
```

**Subtítulos a melhorar (linha 1000):**
- Atual: `subTitle={\`Monitoramento de ${category.replace(/-/g, ' ')}\`}`
- Proposto: Usar mapa com exemplos específicos (ver seção 2.7)
...
### Fase 1+2: Integração Core + Remoção form_schema (SIMULTÂNEO - 5-7 horas)

**EXECUTAR EM PARALELO conforme solicitado pelo usuário.**

#### 2.1 NÃO Criar display_names.py
- **IMPORTANTE**: Não criar arquivo hardcoded
- Display names vêm 100% das regras dinâmicas
...
- Remover funções hardcoded (_infer_category_and_type, _format_display_name, _format_category_display_name)
- MANTER `_extract_blackbox_module()` (necessária para extrair módulo)

#### 2.3 Atualizar categorization_rule_engine.py
- Remover `self.form_schema` da classe `CategorizationRule`
- Manter `_format_display_name()` interno como fallback

#### 2.4 Remover form_schema de categorization_rules.py (FASE 2)
- Remover classes `FormSchemaField` e `FormSchema`
- Remover campo `form_schema` de todos os models (Pydantic)
- Atualizar endpoints POST e PUT para não processar form_schema

#### 2.5 Remover form_schema de MonitoringRules.tsx (FASE 2)
- Remover interfaces `FormSchemaField` e `FormSchema`
- Remover campo `form_schema` da interface `CategorizationRule`
- Modal de edição já está correto (não tem campos form_schema)

#### 2.6 Script de Migração do KV
Script para remover form_schema das regras existentes no KV:

```python
# scripts/migrate_remove_form_schema.py
    ...
    if rules_data and 'rules' in rules_data:
        for rule in rules_data['rules']:
            if 'form_schema' in rule:
                ...
                print(f"Removido form_schema da regra: {rule['id']}")

        await config_manager.put('monitoring-types/categorization/rules', rules_data)
        print(f"Migração concluída: {len(rules_data['rules'])} regras atualizadas")
```

### Fase 3: Adicionar Rotas e Menu (1-2 horas)
