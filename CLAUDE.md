...
6. **Remote Installers** (`backend/core/installers/`)
   ...
   - **Windows**: Multi-connector (SSH/WinRM/PSExec) for Windows Exporter
...
│   └── PrometheusConfig.tsx  # YAML config editor
...
restart-app.bat    # Windows only - starts both servers
```
...
- **Current**: Prometheus YAML editor, metadata fields extraction, installer improvements

See `PHASE*_SUMMARY.md` files for detailed implementation notes.
...
- `paramiko` - SSH for Linux installer
